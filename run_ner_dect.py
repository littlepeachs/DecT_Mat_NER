...
def parse_args():
    ...
    parser.add_argument(
        "--validation_file", type=str, default='datasets/matsciner/test.json', help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
    ...
    args.train_file = "datasets/matsciner/{}shot/{}.json".format(args.shot,args.data_file_seed)
    # Sanity checks
...
def add_label_token_bert(model, tokenizer, label_map):
    ...
    for token in sorted_add_tokens:
        if token.startswith('B-') or token.startswith('I-'):  # 特殊字符
            ...
            embed /= (len(indexes)-count_useless)
