...
# sys.path.append('/home/liwentao/MatSciBERT')
...
def get_matscholar_data(shot,seed):
    data_dir = 'datasets/matsciner/'
    if shot==-1:
        train_X, train_y = parse_file(os.path.join(data_dir, "train.txt"))
    else:
        train_X, train_y = parse_file(os.path.join(data_dir, "{}shot/{}.txt".format(shot,seed)))
    val_X, val_y = parse_file(os.path.join(data_dir, 'dev.txt'))
    test_X, test_y = parse_file(os.path.join(data_dir, 'test.txt'))
    return train_X, train_y, val_X, val_y, test_X, test_y


def get_ner_data(data_name,shot,seed, fold=None, norm=False):
    ...
    if data_name == 'sofc':
        ...
        data =  get_matscholar_data(shot,seed)
    else:
